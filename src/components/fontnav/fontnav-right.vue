    ...
    <span v-if="isLogin == false || isLogin == undefined" class="hk-main-text">
      <el-dropdown :hide-on-click="false" class="hk-myname" style="color: #fff;">
        ...
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item><router-link class="text-blacks"  :to="{path:'/',query:{proid:0}}">发布项目</router-link></el-dropdown-item>
          <el-dropdown-item><router-link class="text-blacks" :to="{path:'/',query:{id:0}}">发布职位</router-link></el-dropdown-item>
          <el-dropdown-item><router-link class="text-blacks" to="/">提问</router-link></el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
      <span class="hk-top-r-text padding-lr-12 font-14" style="border:none;"><router-link class="head"  to="/">找人才</router-link></span>
      <el-button class="hk-right-button" @click="onOrgService">企业服务</el-button>
      <span class="hk-top-r-text padding-lr-12 hk-top-r-text-fabu font-14"><a class="head" @click="onLogin">登录</a></span>
      <span class="font-14">|</span>
      <span class="hk-top-r-text padding-l-12 font-14"><a class="head" @click="onRegister">注册</a></span>
    </span>
    <!-- 登录状态 -->
    <span v-else class="">
      <!-- 登录个人端状态 -->
        ...
        <span class="">
          <el-dropdown :hide-on-click="false" class="hk-myname" style="color: #fff;">
            ...
            <el-dropdown-menu slot="dropdown" class="hk-person-news" style="width: 200px;">
              <router-link  style="color: #000;" class="infocenter"  :to="{path:'/'}"><el-dropdown-item>消息</el-dropdown-item></router-link>
            </el-dropdown-menu>
          </el-dropdown>
...
export default {
  ...
  methods: {
      ...
      logout().then( res => {
        if (res.status == 200) {
          ...
          if (fullPath.indexOf('/user/person') != -1) {
              ...
              window.location.href = urlConfig.accountModulUrl+'account/register'+'?oauth_callback=' + url;
          }
    ...
    onLogin(){
      let url = encodeURIComponent(window.location.href)
      window.location.href = urlConfig.accountModulUrl+'account/login'+'?oauth_callback=' + url;
    },
    onOrgService(){
