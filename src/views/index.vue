<template>
  <div class="stoer" v-loading.fullscreen.lock="fullscreenLoading">
    <storeHeader class="store-main-nav" :menusList="menusList"></storeHeader>
    <div class="content">
    ...
    <div class="hot-selling-items content">
      ...
      <ul class="hot-selling margin-b-20">
        <hotGoods v-for="(item,index) in benefit" :item="item" :key="index"></hotGoods>
      </ul>
    </div>
  </div>
</template>
...
import {getCombinationList, getGroomList, getHostProducts} from "@/api/store"
...
export default {
  ...
  methods: {
    ...
    get_host_products(){
      ...
      getHostProducts(parameter).then(res=>{
    ...
    get_home_data() {
      ...
      getHomeData().then(res => {
        if (res.status === 200) {
          ...
          that.benefit = res.data.benefit
          that.menusList = res.data.menus
        }
      }).catch(res => {
        console.log(res)
      })
